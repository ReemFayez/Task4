...
APIKey = IAMAuthenticator("j_3Fcx7iefRw6TdAA-iF6wvNzDFHtxCfP7u0cvZpJrb4")
tts = TextToSpeechV1(authenticator=APIKey)
tts.set_service_url("https://api.us-south.text-to-speech.watson.cloud.ibm.com/instances/c23cdd4c-4946-455b-8a8c-819002120ae2")
...
#with open('input-speech.txt','r') as f:
 #   MyText=f.readlines()
  #  MyText=[line.replace('\n','') for line in MyText]
   # MyText=''.join(str(line) for line in MyText)
...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        with open('./output.mp3', 'wb') as AuFile:
             x = "Hello, how are you?"
             result =tts.synthesize(x, accept='audio/mp3', voice='en-US_AllisonV3Voice').get_result()
             AuFile.write(result.content)
